"""
run_pipeline.py — End-to-end Time Series Forecasting Pipeline.
...
from src.models import (NaiveForecaster, MovingAverageForecaster, ARIMAForecaster,
...
def run(data_path=None, store_id=None, product_id=None):
    print("\n" + "="*62)
    ...
    if data_path and os.path.exists(data_path):
        ...
        if store_id and "store_id" in df.columns: df = df[df["store_id"] == store_id]
        if product_id and "product_id" in df.columns: df = df[df["product_id"] == product_id]
        if len(df) == 0: raise ValueError("No rows after filtering.")
    else:
    ...
    try:
        ...
        eval_results["ARIMA"] = {"actual": test_series.values, "predicted": arima_model.predict(n_test)}
    ...
    model_map = {"Naive": naive, "MovingAverage": ma, "ARIMA": arima_model if arima_model else ma,
    ...
    generate_html_report(eda_results=eda_results, metrics_df=metrics_df, best_model_name=best_model_name,
                         forecast_df=forecast_df, eval_figs=eval_figs, forecast_fig=forecast_fig, output_path=report_path)
