...
report.py
=========
Self-contained HTML report generator (PRD Deliverable: HTML report).
Produces a Bootstrap-styled report with:
  1. Executive Summary + KPI cards
  2. EDA charts (trend, seasonality, decomposition, ADF)
  3. Model comparison table + metric charts + residuals
  4. 30-day forecast chart + forecast table
"""
import base64
import io
import os
...
def _img(b64: str, width: str = "100%") -> str:
    return (f'<img src="data:image/png;base64,{b64}" '
            f'style="width:{width};max-width:960px;margin:10px 0;" />')
...
def generate_html_report(
    ...
    Args:
        eda_results:     Output of eda.run_eda()
        metrics_df:      Output of evaluation.evaluate_all()
        best_model_name: Name of the best-performing model
        forecast_df:     30-day forecast DataFrame
        eval_figs:       {predictions_fig, metrics_fig, residuals_fig}
        forecast_fig:    Forecast plot figure
    ...
    crit_rows = "".join(
        f"<tr><td>Critical Value ({k})</td><td>{v}</td></tr>"
...
<head>
  ...
  <style>
    ...
    h2{{font-size:1.35rem;font-weight:700;color:#34495e;
        border-left:4px solid #4E79A7;padding-left:10px;margin-top:38px;}}
    ...
    .kpi-box{{background:#fff;border-radius:8px;padding:14px 18px;min-width:130px;
...
<div class="wrap">
  ...
  <div class="card">
    <p>Historical retail sales data was cleaned, analysed for trend and seasonality, and used to train five
    forecasting models. The best-performing model — <strong>{best_model_name}</strong> (lowest RMSE on a
    held-out 30-day test window) — was used to generate the final 30-day demand forecast.</p>
    <div class="kpi">
    ...
    <p>
      <strong>Best Model Metrics:</strong>
      &nbsp;RMSE&nbsp;=&nbsp;<strong>{best_row['RMSE']}</strong>
      &nbsp;|&nbsp;MAE&nbsp;=&nbsp;<strong>{best_row['MAE']}</strong>
      &nbsp;|&nbsp;MAPE&nbsp;=&nbsp;<strong>{best_row['MAPE (%)']:.2f}%</strong>
    </p>
  </div>
  ...
  <div class="card">
    ...
    <h3>2.3 Seasonal Decomposition (Weekly Period)</h3>
    {_img(decomp_b64)}

    <h3>2.4 Feature Correlation with Sales</h3>
    {_img(corr_b64, "50%")}

    <h3>2.5 Stationarity — Augmented Dickey-Fuller Test</h3>
    ...
    <p style="color:#7f8c8d;font-size:.82rem;margin-top:8px;">
      H₀: Series has a unit root (non-stationary). p &lt; 0.05 → reject H₀ → stationary.
    </p>
  </div>
  ...
  <div class="card">
    <p>Forward forecast generated using <strong>{best_model_name}</strong> (best RMSE on test set).
    Shaded band represents ±10% uncertainty range.</p>
    {_img(forecast_b64)}

    <h3>Forecast Values (First 10 Days)</h3>
    {forecast_table}
    <p style="color:#7f8c8d;font-size:.82rem;">Full 30-day forecast exported to
    <code>outputs/forecasts/</code> as CSV.</p>
  </div>

  <div class="footer">
    Time Series Forecasting Pipeline &nbsp;|&nbsp; Generated {now}
  </div>
</div>
</body>
</html>"""
