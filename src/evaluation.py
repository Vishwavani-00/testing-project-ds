...
evaluation.py
=============
Model evaluation utilities (PRD 7.5):
  Metrics: MAE, RMSE, MAPE
  Charts:  predictions vs actual, metrics bar chart, residual analysis
"""
import warnings
...
def evaluate_all(results: dict) -> pd.DataFrame:
    """
    Compute MAE, RMSE, MAPE for every model in results dict.

    Args:
        results: {model_name: {"actual": arr, "predicted": arr}}
    ...
    for name, vals in results.items():
        a = np.array(vals["actual"])
        p = np.array(vals["predicted"])
        rows.append({
            "Model":    name,
            "MAE":      round(mae(a, p), 4),
...
def plot_predictions(results: dict,
                     test_dates: pd.Series) -> plt.Figure:
    """Line plot: all model predictions vs actual on test set."""
    fig, ax = plt.subplots(figsize=(13, 5))
    actual = list(results.values())[0]["actual"]
    ax.plot(test_dates, actual, color="black", linewidth=2.2,
            label="Actual", zorder=5)

    for i, (name, vals) in enumerate(results.items()):
        ax.plot(test_dates, vals["predicted"],
                color=PALETTE[i % len(PALETTE)], linewidth=1.5,
                linestyle="--", alpha=0.85, label=name)

    ax.set_title("Model Predictions vs Actual — Test Set",
...
def plot_metrics_comparison(metrics_df: pd.DataFrame) -> plt.Figure:
    """Side-by-side bar charts for MAE, RMSE, MAPE."""
...
def plot_residuals(results: dict, best_model: str) -> plt.Figure:
    """Residual time plot + histogram for the best model."""
    vals      = results[best_model]
    residuals = np.array(vals["actual"]) - np.array(vals["predicted"])

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))

    axes[0].plot(residuals, color=PALETTE[0], linewidth=0.9)
