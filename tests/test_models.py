...
class TestNaive:
    def test_length(self, series):
        assert len(NaiveForecaster().fit(series).predict(10)) == 10

    def test_constant(self, series):
        preds = NaiveForecaster().fit(series).predict(5)
        assert np.allclose(preds, preds[0])

    def test_no_negatives(self, series):
        assert (NaiveForecaster().fit(series).predict(30) >= 0).all()


class TestMovingAverage:
    def test_length(self, series):
        assert len(MovingAverageForecaster().fit(series).predict(10)) == 10

    def test_no_negatives(self, series):
        assert (MovingAverageForecaster().fit(series).predict(30) >= 0).all()
